...
import io,urllib,base64,random,time
import matplotlib.pyplot as plt
from skripsi.scripts.c45 import selection, create_rule
from skripsi.scripts.confusion_matrix import accuracy,confusion_matrix
from skripsi.scripts.pengujian import k_fold_cross_validation


def feature_selection(dataset,instance,atribut,atrNumerik,jumPartikel,iterasi,w,c1,c2,iter_param):
    # INIT
    ...
    for i in range(iterasi):
        pbest,gbest = update_gbest_pbest(x,gbest,pbest,jumPartikel,dataset,instance,atribut,atrNumerik,iter_param,i+1)
...
def update_gbest_pbest(x,gbest,pbest,jumPartikel,dataset,instance,atribut,atrNumerik,iter_param,iter_pso):
    for i in range(jumPartikel):
        # hitung fitness (C4.5)
        ...
        print(iter_param,'.',iter_pso,'.',i+1,'.','WAKTU KFCV :',round(time.perf_counter()-start,5),'='*random.choice(range(10,50)))
        # print('[',round(time.perf_counter()-start,5),']', end ="-")
